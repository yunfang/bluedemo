...
import com.uyu.device.library.BluetoothService;
...
public class BluetoothSPP {
    ...
    public void startService(boolean isAndroid) {
        if (mChatService != null) {
            if (mChatService.getState() == com.uyu.device.library.BluetoothState.STATE_NONE) {
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
            case com.uyu.device.library.BluetoothState.MESSAGE_WRITE:
                break;
            case com.uyu.device.library.BluetoothState.MESSAGE_READ:
                ...
                break;
            case com.uyu.device.library.BluetoothState.MESSAGE_DEVICE_NAME:
                mDeviceName = msg.getData().getString(com.uyu.device.library.BluetoothState.DEVICE_NAME);
                mDeviceAddress = msg.getData().getString(com.uyu.device.library.BluetoothState.DEVICE_ADDRESS);
                if(mBluetoothConnectionListener != null)
                    mBluetoothConnectionListener.onDeviceConnected(mDeviceName, mDeviceAddress);
                isConnected = true;
                break;
            case com.uyu.device.library.BluetoothState.MESSAGE_TOAST:
                Toast.makeText(mContext, msg.getData().getString(com.uyu.device.library.BluetoothState.TOAST)
                        , Toast.LENGTH_SHORT).show();
                break;
            case com.uyu.device.library.BluetoothState.MESSAGE_STATE_CHANGE:
                if(mBluetoothStateListener != null)
                    mBluetoothStateListener.onServiceStateChanged(msg.arg1);
                if(isConnected && msg.arg1 != com.uyu.device.library.BluetoothState.STATE_CONNECTED) {
                    if(mBluetoothConnectionListener != null)
                ...
                if(!isConnecting && msg.arg1 == com.uyu.device.library.BluetoothState.STATE_CONNECTING) {
                    isConnecting = true;
                } else if(isConnecting) {
                    if(msg.arg1 != com.uyu.device.library.BluetoothState.STATE_CONNECTED) {
                        if(mBluetoothConnectionListener != null)
    ...
    public void connect(Intent data) {
        String address = data.getExtras().getString(com.uyu.device.library.BluetoothState.EXTRA_DEVICE_ADDRESS);
    ...
    public void send(byte[] data, boolean CRLF) {
        if(mChatService.getState() == com.uyu.device.library.BluetoothState.STATE_CONNECTED) {
            if(CRLF) {
